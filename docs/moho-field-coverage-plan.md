...
| M1.5.1 | Bone IK / constraints (**12 real keys, re-scoped 2026-08-19** — not the stale "remainder (14)" above) | **59.2% (257/434)** | **DONE** | `6265bd8`, `10a4ac7` |
| M1.5.2 | Bone dynamics (`pos_*`/`scale_*`) + rigid-body Physics (27) | **65.4% (284/434)** | **DONE** | `54760d4`, `ab78ba0` |
| M1.5.3 | ParticleLayer / Physics / Crayon (20, 2 multi-owner) | **70.0% (304/434)** | **DONE** | `19e1e78`, `e224312` |
| M1.5.4 | TextLayer / NoteLayer text+balloon (24, 1 multi-owner) | **75.6% (328/434)** | **DONE** | `02fb580` |
...
preconditions as separate late milestones) never actually ran as planned —
re-scoping every M1.x milestone against the live registry (the same
correction M1.1 needed) showed the true remaining-key groupings didn't
match the plan's original guesses, so M1.5 absorbed the real work as eight
freshly-scoped batches (M1.5.1–M1.5.8 above) instead. **The 95% target was
reached at M1.5.7 — 417/434 = 96.1%, 0 registry/target failures,
independently re-verified twice (once per review, once after a fix
round).** The 3D family (13 keys under `Mesh3DOptions`/`MeshLayer`/`Shape`)
was deliberately left unattempted at that point — a completeness/quality
pass the user chose to pursue anyway once the target was already met — and
is now closed by M1.5.8: a genuine compound precondition was found (see the
Q8 resolution below), and all 13 keys registered `EDITABLE` with a real
Moho-render probe apiece, landing at **430/434 = 99.1%**.
...
| Q8 | **M3.1's efficiency premise is measurably false, and this changes the plan's arithmetic.** The plan claims "one `3d_mode = 1` precondition unlocks all ten `3d_*` keys". Task 10's probe measured it on `Bandit.mohoproj` frame 25 and I re-ran it independently: `3d_mode = 1` alone **does** change the render (21 sites, AFFECTS RENDER), so the precondition is genuinely live — but `3d_shading_density = 90` behind that same precondition is still **inert**. The precondition works and does not unlock the target key. So the ten `3d_*` keys need individual, possibly **compound** preconditions (`3d_shading_mode`, a non-zero `3d_thickness`, or a mesh that actually extrudes), not one shared switch. Consequence for the target: M3 must supply 31 of the 39 constant-valued keys, and 10 of those 31 were budgeted as this cheap group. If they resist, the 8-key residual cannot absorb a 10-key loss, and the 95% target would miss. **M3.1 must therefore begin by finding a working compound precondition for the `3d_*` family and reporting whether one exists, before any of its ten keys is scheduled.** | **CLOSED (M1.5 batch 8).** A working compound precondition WAS found, resolving the "possibly compound" question this row raised: `3d_mode=1` alone unlocks six of the ten `Mesh3DOptions` fields (`3d_shading_mode`, `3d_crease_edges`, `3d_crease_angle`, `3d_edge_extension`, `3d_backface_removal`, `3d_reset_z`), and `3d_mode=1` **+** `3d_shading_mode=2` (guessed Toon, per Moho's own scripting header's `Mc_SHADING_TOON` constant and the manual's 5-choice shading-mode list) unlocks the remaining four (`3d_shading_density`, `3d_shading_color`, `3d_silhouette_edges`, `3d_material_edges`) — each of those four was independently re-confirmed inert under `3d_mode=1` alone first, so this is a measured correction, not a guess accepted on faith. The non-zero-`3d_thickness` candidate this row named turned out not to be the gate at all: every sampled shape already carries a non-zero constant `3d_thickness=0.125` corpus-wide, yet `Mesh3DOptions` was still inert under `3d_mode=0` — `3d_mode` is the real gate; `3d_thickness`'s own magnitude only becomes visible once that gate is open (confirmed: forcing `3d_thickness=1.0` is inert under `3d_mode=0` and AFFECTS RENDER under `3d_mode=1`). All 13 keys (10 `Mesh3DOptions` sub-fields + the `3d_options` container + `3d_mode` + the per-shape `3d_thickness`) are now `EDITABLE`, all AFFECTS RENDER, all with a direct-PNG visual confirmation (not just a pixel-diff count) — see `docs/moho-project-file-format.md` § 6.4's `Mesh3DOptions` entry and `docs/moho-field-probes.md` for every probe row. |
