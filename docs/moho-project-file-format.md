...
- `noisy_lines`, `noisy_shapes`, `extra_sketchy`, `extra_lines`, `noise_amp`,
  ...
  **M1.5 batch 5 — the rest of this family is now measured, and NOT
  uniformly inert.** A first probe round forcing each flag from its never-
  authored-true state on Bandit.mohoproj (which has every one of these
  fields at its off/default value) found ALL of `noisy_lines`,
  `noisy_shapes`, `extra_sketchy`, `extra_lines`, `noise_interval` and
  `animated_noise` inert — a suspiciously uniform result the brief itself
  warned against trusting from a document that never authors the effect.
  A second round on documents that DO carry the flag natively true
  (`DonkeyAndMan.mohoproj` for `noisy_lines`/`noisy_shapes`/`extra_sketchy`/
  `animated_noise`, all on the same 2 layers; `Others/TargetBone.animeproj`
  for `noise_interval`; `Others/AddBone.animeproj` for `extra_lines`,
  flipping the real `true`/`2`/`5` values back to their off/default state)
  reversed three of the six: **`noisy_shapes`, `animated_noise` and
  `noise_interval` each AFFECT RENDER**; `noisy_lines`, `extra_sketchy` and
  `extra_lines` stayed inert even on the real, actively-authored fixture —
  a genuine, twice-confirmed negative for those three, not a testing-the-
  wrong-document artefact. All eight fields (this bullet's own six plus the
  already-`UNKNOWN` `noise_amp`/`noise_scale` pair, still without a writer)
  are now registered in `schema/layer.schema.json`'s `MeshLayer` $def; see
  each field's own `x-moho-evidence` for the exact fixture/frame.
- `gap_filling` — **M1.5 batch 5:** constant `false` on every one of 4,969
  corpus occurrences (76 documents), genuinely universal unlike its sketchy-
  line siblings above (none of which is universally off). Forced `true` on
  all 21 sites of Bandit.mohoproj at frame 25 (no real-usage document exists
  corpus-wide): inert. Now `EDITABLE`.
- `antialiasing` (`exclude_lines_from_mask` is decoded but
...
- `triangulated`, `squashable_deformer`, `frame_zero_deformer` — three
  deformer flags that exist **only in the `1045` format generation**.
  **Corrected (M1.5 batch 5):** an earlier revision of this bullet claimed
  `false`/`false`/`true` "throughout" (all 21 `MeshLayer`s of
  `Bandit.mohoproj`; absent from every `1038` and `1021` layer) — that
  undersold the corpus: a 76-document scan of every `1045` document found
  `triangulated` true on 16 of 297 sites across 5 documents (`Boar`,
  `Lute`, `Night_Boy`, `Spacewoman`, `Whale`) and `frame_zero_deformer`
  false on 140 of 248 sites across all 10 `1045` documents (Bandit's own 21
  sites ARE true, matching the old claim for that one document only).
  `squashable_deformer` remains `false` in every sample checked so far.
  M1.5 batch 5 forced `triangulated` true→false on Whale.mohoproj's own
  real active sites (inert, confirmed twice against an earlier Bandit-forced
  probe) and `frame_zero_deformer` true→false on Bandit.mohoproj's own real
  active sites (inert) — both now `EDITABLE`. They are the clearest sign in
...
**M1.5 batch 5 — `direction` under both `gravity` and `wind`.** 51
`gravity.direction` sites across 10 documents (constant `4.712389` rad,
straight down) and 55 `wind.direction` sites across 11 documents (mostly
`0.0`) — corrected corpus counts, since the schema's own pre-existing
description of both fields still said "rare - observed on exactly one
BoneLayer" (a stale, pre-M1.4a-style claim fixed this session). Probed by
reproducing the exact precondition `Skeleton.dynamic_angles`' own WIND
EVIDENCE section already used on an untracked synthetic two-bone rig
(`wind.strength = 100`, turbulence matching `DarkMan.mohoproj`'s own values,
a bone subscribed via `wind_dynamics = true`) — but this time on
`Bandit.mohoproj`, a real corpus document that already stores those exact
`wind.strength`/`turbulence_amplitude`/`turbulence_frequency` values
natively: baking `wind_dynamics = true` onto all 28 bones and varying
`wind.direction` (isolated from `gravity`'s own occurrence) is **inert**,
and the same precondition plus `gravity.strength` forced non-zero (`0.0` →
`50.0`, needed since gravity is off by default on Bandit) varying
`gravity.direction` is **also inert**. Both results corroborate the
existing WIND EVIDENCE finding (Moho 14.4 rendered the prior minimal rig
twice, wind subscribed and not, and the driven bone's rotation differed by
0.0000 degrees) on a different, real document rather than re-deriving it —
this is the documented, expected outcome for a mechanism this repository's
own module docstring already confirms does not reproduce Moho's real
spring-damper effect under the tested setup, not a new gap. Both fields are
now `EDITABLE`.
...
- `ImageLayer`'s `toon_*` cel-shading fields, `sampling_mode`,
  ...
  finding.)** ([§ 6.5](#65-imagelayer-19-file-finding))
- `SS_Soft`/`SS_Shadow`/`SS_Texture2` internals — `fill_mode`, and the
  interplay between a shape's `fill_style` and `fill_style2` when both are
  present. **(19-file finding.)** ([§ 8.3](#83-style-effect-variants-46-file-finding)).
  **`SS_Crayon`'s own `rand_seed`/`clear_background`/`reduce_randomization`
  are no longer in this list — M1.5 batch 5 probed all three on
